...
import { useCertificates, useCreateCertificate, useUpdateCertificate, useDeleteCertificate, useUploadCertificateImage } from '../../hooks/useData';
...
const AdminCertificates: React.FC = () => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            try {
                ...
                setCurrentCert({ ...currentCert, image_url: result.url });
            } catch (err) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (currentCert._id) {
                await updateCert.mutateAsync({ id: currentCert._id, data: currentCert });
            } else {
                await createCert.mutateAsync(currentCert as Omit<Certificate, '_id'>);
            }
            setIsEditing(false);
            setCurrentCert({ title: '', issuer: '', date: new Date().toISOString().split('T')[0], image_url: '', verification_url: '' });
        } catch (err) {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-10">
            ...
            <div className="bg-white rounded-2xl border border-ink-100 p-8 mb-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 animate-up">
                <div>
                    <div className="section-badge mb-3">Achievements</div>
                    <h1 className="text-3xl font-extrabold text-ink-900 mb-2">Manage Certificates</h1>
                    <p className="text-ink-500 font-medium text-sm">Add and update your professional certifications.</p>
                </div>
                <button
                    onClick={() => {
                        setCurrentCert({ title: '', issuer: '', date: new Date().toISOString().split('T')[0], image_url: '', verification_url: '' });
                ...
                <div className="fixed inset-0 backdrop-blur-md bg-slate-900/20 flex items-center justify-center p-4 z-50 overflow-y-auto">
                    <div className="bg-white rounded-2xl border-2 border-ink-200 w-full max-w-2xl p-8 my-8 max-h-[90vh] overflow-y-auto animate-up">
                        <h2 className="text-xl font-bold mb-6">
                            {currentCert._id ? 'Edit Certificate' : 'Add Certificate'}
                        </h2>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-bold text-ink-700 mb-1.5">Certificate Image</label>
                                    <div className="flex items-center gap-6">
                                        <div className="w-32 h-24 bg-ink-50 rounded-xl overflow-hidden border border-ink-100 flex-shrink-0">
                                            {currentCert.image_url ? (
                                                <img src={resolveMediaUrl(currentCert.image_url)} alt="Preview" className="w-full h-full object-cover" />
                                            ) : (
                                                <div className="w-full h-full flex items-center justify-center text-ink-300">📜</div>
                                            )}
                                        </div>
                                        <div className="flex-grow">
                                            <input type="file" onChange={handleImageUpload} className="hidden" id="cert-image" />
                                            <label htmlFor="cert-image" className="btn-ghost !py-2.5 cursor-pointer inline-flex">
                                                {uploadImage.isPending ? 'Uploading...' : 'Choose Image'}
                                            </label>
                                            <p className="text-[10px] text-ink-400 mt-2">Recommended: Landscape (4:3) image</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-bold text-ink-700 mb-1.5">Title</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-ink-700 mb-1.5">Issuer</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-ink-700 mb-1.5">Date Issued</label>
                                    <input
                                        type="date"
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-bold text-ink-700 mb-1.5">Verification URL (Optional)</label>
                                    <input
                                        type="url"
                                        value={currentCert.verification_url}
                                        onChange={(e) => setCurrentCert({ ...currentCert, verification_url: e.target.value })}
                                        className="w-full px-4 py-3 border border-ink-200 rounded-xl focus:ring-2 focus:ring-accent-500 outline-none transition-all"
                                    />
                                </div>
                            </div>
                            <div className="flex justify-end gap-3 pt-6 border-t border-ink-100">
                                ...
                                <button type="submit" disabled={!currentCert.image_url} className="px-10 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all disabled:bg-ink-200">
                                    {currentCert._id ? 'UPDATE CERTIFICATE' : 'CREATE CERTIFICATE'}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <div className="grid grid-cols-1 gap-5">
                {certificates?.map((cert, i) => (
                    <div key={cert._id} className="card group flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 animate-up" style={{ animationDelay: `${i * 50}ms` }}>
                        <div className="flex items-center gap-6">
                            <div className="w-32 bg-ink-50 rounded-xl overflow-hidden border border-ink-100 flex-shrink-0">
                                <img src={resolveMediaUrl(cert.image_url)} alt="" className="w-full h-auto" />
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-ink-900">{cert.title}</h3>
                                <p className="text-sm text-ink-500 font-medium">{cert.issuer} • {new Date(cert.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short' })}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-0 border-ink-50">
                            <button onClick={() => handleEdit(cert)} className="flex-1 sm:flex-none p-3 text-blue-600 hover:bg-blue-50 rounded-xl transition-colors">
                            ...
                            <button onClick={() => cert._id && handleDelete(cert._id)} className="flex-1 sm:flex-none p-3 text-red-600 hover:bg-red-50 rounded-xl transition-colors">
            ...
            {certificates?.length === 0 && (
                <div className="text-center bg-white rounded-2xl border-2 border-dashed border-ink-100 py-16 animate-up">
                    <p className="text-ink-400 font-medium italic">No certificates added yet.</p>
                </div>
            )}
        </div>
    );
